...
function tryInit(fn, name) {
  try { fn(); } catch (e) {
    ...
    log(`Init warning: ${name} — ${e.message}`, 'warn');
  }
}
...
async function init() {
  ...
  tryInit(initCollapsibles,       'initCollapsibles');
  tryInit(initTopoUpload,         'initTopoUpload');
  tryInit(initLabImport,          'initLabImport');
  tryInit(initGeoMapImport,       'initGeoMapImport');
  tryInit(initReset,              'initReset');
  tryInit(initApiKeyModal,        'initApiKeyModal');
  tryInit(initCertaintySlider,    'initCertaintySlider');
  tryInit(initTransparencyControls, 'initTransparencyControls');
  tryInit(initSurfaceOpacity,     'initSurfaceOpacity');
  tryInit(initVerticalExaggeration, 'initVerticalExaggeration');
  tryInit(initBHSticksToggle,     'initBHSticksToggle');
  tryInit(initConstraints,        'initConstraints');
  tryInit(initRunAI,              'initRunAI');
  tryInit(initBuildModel,         'initBuildModel');
  tryInit(initViewModeButtons,    'initViewModeButtons');
  tryInit(initVBHButton,          'initVBHButton');
  tryInit(initIsopachMap,         'initIsopachMap');
  tryInit(_initStratLockToggle,   '_initStratLockToggle');
  tryInit(initStratOrderButtons,  'initStratOrderButtons');
  tryInit(_initGeoEventTimeline,  '_initGeoEventTimeline');
  tryInit(initFenceSection,       'initFenceSection');
  tryInit(initScreenshot,         'initScreenshot');
  tryInit(initBackgroundToggle,   'initBackgroundToggle');
  tryInit(initMeasureTool,        'initMeasureTool');
  tryInit(initModelReport,        'initModelReport');
  tryInit(initAnnotations,        'initAnnotations');
  tryInit(initPlanView,           'initPlanView');
  tryInit(initStratCorrelation,   'initStratCorrelation');
  tryInit(initPropertiesTab,      'initPropertiesTab');
  tryInit(initLegendRename,       'initLegendRename');
  tryInit(initTopoClip,           'initTopoClip');
  tryInit(initCursorCoords,       'initCursorCoords');
  tryInit(initShortcutsModal,     'initShortcutsModal');
  tryInit(initGWT,                'initGWT');
  tryInit(initCameraPresets,      'initCameraPresets');
  tryInit(initViewBookmarks,      'initViewBookmarks');
  tryInit(initVolumeByDepth,      'initVolumeByDepth');
  tryInit(initGradeShell,         'initGradeShell');
  tryInit(initCrossPlot,          'initCrossPlot');
  tryInit(initVariogramControls,  'initVariogramControls');
  tryInit(initCRSTools,           'initCRSTools');
  tryInit(initSession,            'initSession');
  tryInit(initProjectConfig,      'initProjectConfig');
  tryInit(initInterpretGeology,   'initInterpretGeology');
  tryInit(initSettlement,         'initSettlement');
  tryInit(initBearingCapacity,    'initBearingCapacity');
  tryInit(initPileCapacity,       'initPileCapacity');
  tryInit(initColorPresets,       'initColorPresets');
  tryInit(initAutoParams,         'initAutoParams');
  tryInit(initUnitEditor,         'initUnitEditor');
  tryInit(initRiskAssessment,     'initRiskAssessment');
  tryInit(initSemanticModel,      'initSemanticModel');
  tryInit(initParameterView,      'initParameterView');
  tryInit(initIsosurfaces,        'initIsosurfaces');
  tryInit(initScenarioManager,    'initScenarioManager');
  tryInit(initOrientationImport,  'initOrientationImport');
  tryInit(initFoundationExport,   'initFoundationExport');
  tryInit(initBHLogView,          'initBHLogView');
  tryInit(initLogSubTabs,         'initLogSubTabs');
  tryInit(initSPTProfile,         'initSPTProfile');
  tryInit(initBHDataSubTabs,      'initBHDataSubTabs');
  tryInit(initCPTImport,          'initCPTImport');
  tryInit(initWelcomeOverlay,     'initWelcomeOverlay');
  tryInit(initStereonet,          'initStereonet');
  tryInit(initSlopeStability,     'initSlopeStability');
  tryInit(initGeoFeatures,        'initGeoFeatures');
  tryInit(initLiquefaction,       'initLiquefaction');
  tryInit(initMohrCircle,         'initMohrCircle');
